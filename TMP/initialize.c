...
*/
	...
	*( (int *)pptr->pbase ) = MAGIC;
	...
	pptr->vmemlist = NULL;
	for (i = 0; i < 16; i++) {
		pptr->bs_list[i].ispriv = 0;
		pptr->bs_list[i].bs_npages = 0;
		pptr->bs_list[i].bs_pid = -1;
		pptr->bs_list[i].bs_refcnt = 0;
		pptr->bs_list[i].bs_sem = -1;
		pptr->bs_list[i].bs_status = UNMAPPED;
		pptr->bs_list[i].bs_vpno = -1;
	}
	...
	if(page_replace_policy==SC)
  	{
   		 head=(struct cir_queue*)getmem(sizeof(struct cir_queue));
   		 head->fr_no=-1;
   		 head->pid=currpid;
     	  	 head->next = head;
   		 curr = head;
  	}	
	init_global_pt(NULLPROC);
...
void pt_init(){
	...
	for(i = 0; i < 4; i++)
		...
		frm_tab[fr_no].fr_status = MAPPED;
		frm_tab[fr_no].fr_pid = NULLPROC;
		page_entry = (FRAME0+fr_no)*NBPG;
		for(j = 0; j < 1024; j++)
			{
...
int init_global_pt() {
	...
	int new_pt;
        for (i = 0; i < 4; i++) {
                new_pt = create_PageTable(NULLPROC);
                if (new_pt == -1) {
                        return SYSERR;
                }
                global_pt[i] = FRAME0 + new_pt;
